...
> **DESACTIVADO a pedido del usuario (2026-07-15):** se eliminó la
> puerta de página completa `_club_gate` (secret `club_password`) y
> `app.py` fuerza `IS_ADMIN = True` sin montar `auth.login_entry()` —
> la app queda pública SIN contraseña, ingesta incluida (riesgo
> aceptado: cualquiera con la URL puede cargar/borrar resultados y
> sincronizar al repo). `auth.py` y sus tests quedan intactos; para
> reactivar, restaurar las líneas marcadas "spec §8" en `app.py`.
